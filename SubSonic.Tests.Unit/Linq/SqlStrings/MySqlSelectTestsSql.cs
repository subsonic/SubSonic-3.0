...
{
	public class MySqlSelectTestsSql : ISelectTestsSql
	{
...
WHERE ((`t0`.`CustomerID` = 'ABCDE') OR (`t0`.`CustomerID` = 'TEST1'))";
		...
		public string Contains_Resolves_Literal
		{
			get
			{
				return @"SELECT `t0`.`Address`, `t0`.`City`, `t0`.`CompanyName`, `t0`.`ContactName`, `t0`.`Country`, `t0`.`CustomerID`, `t0`.`Region`
FROM `Customers` AS t0
WHERE (`t0`.`ContactName` LIKE CONCAT('%','har','%'))";
			}
		}
...
WHERE `t0`.`CustomerID` IN (
  ...
  FROM `Orders` AS t1
		...
		public string Count_Distinct
		{
			get
			{
				return @"SELECT DISTINCT `t0`.`City`
FROM `Customers` AS t0";
			}
		}
...
FROM `Orders` AS t0";
		...
		public string Distinct_Should_Not_Fail
		{
			get
			{
				return @"SELECT DISTINCT `t0`.`Address`, `t0`.`City`, `t0`.`CompanyName`, `t0`.`ContactName`, `t0`.`Country`, `t0`.`CustomerID`, `t0`.`Region`
FROM `Customers` AS t0";
			}
		}
...
INNER JOIN `Customers` AS t0
  ON ((`t0`.`City` IS NULL AND `t2`.`City` IS NULL) OR (`t0`.`City` = `t2`.`City`))";
		...
		public string GroupBy_Sum
		{
			get
			{
				return @"SELECT SUM(`t0`.`OrderID`) AS agg1, MIN(`t0`.`OrderID`) AS agg2, MAX(`t0`.`OrderID`) AS agg3, AVG(`t0`.`OrderID`) AS agg4
FROM `Orders` AS t0
GROUP BY `t0`.`CustomerID`";
			}
		}
		...
		public string GroupBy_Sum_With_Element_Selector_Sum_Max
		{
			get
			{
				return @"SELECT SUM(`t0`.`OrderID`) AS agg1, MAX(`t0`.`OrderID`) AS agg2
FROM `Orders` AS t0
GROUP BY `t0`.`CustomerID`";
			}
		}
		...
		public string GroupBy_Sum_With_Result_Selector
		{
			get
			{
				return "SELECT SUM(`t0`.`OrderID`) AS c0, MIN(`t0`.`OrderID`) AS c1, MAX(`t0`.`OrderID`) AS c2, AVG(`t0`.`OrderID`) AS c3\r\nFROM `Orders` AS t0\r\nGROUP BY `t0`.`CustomerID`";
			}
		}
		...
		public string GroupBy_With_Anon_Element
		{
			get
			{
				return @"SELECT SUM(`t0`.`OrderID`) AS agg1
FROM `Orders` AS t0
GROUP BY `t0`.`CustomerID`";
			}
		}
...
FROM `Products` AS t0";
		...
		public string Select_Anon_Nested
		{
			get
			{
				return "SELECT `t0`.`ProductName`, `t0`.`UnitPrice`\r\nFROM `Products` AS t0";
			}
		}
		...
		public string Select_Anon_One_And_Object
		{
			get
			{
				return "SELECT `t0`.`ProductName`, `t0`.`CategoryID`, `t0`.`Discontinued`, `t0`.`ProductID`, `t0`.`Sku`, `t0`.`UnitPrice`\r\nFROM `Products` AS t0";
			}
		}
		...
		public string Select_Anon_Three
		{
			get
			{
				return "SELECT `t0`.`ProductName`, `t0`.`UnitPrice`, `t0`.`Discontinued`\r\nFROM `Products` AS t0";
			}
		}
...
FROM `Products` AS t0";
		...
		public string SelectMany_Customer_Orders
		{
			get
			{
				return "SELECT `t0`.`ContactName`, `t1`.`OrderID`\r\nFROM `Customers` AS t0\r\nCROSS JOIN `Orders` AS t1\r\nWHERE (`t0`.`CustomerID` = `t1`.`CustomerID`)";
			}
		}
...
WHERE (`t0`.`City` LIKE CONCAT('%','10'))";
		...
		public string Where_Resolves_String_EndsWith_OtherColumn
		{
			get
			{
				return @"SELECT `t0`.`Address`, `t0`.`City`, `t0`.`CompanyName`, `t0`.`ContactName`, `t0`.`Country`, `t0`.`CustomerID`, `t0`.`Region`
FROM `Customers` AS t0
WHERE (`t0`.`ContactName` LIKE CONCAT('%',`t0`.`ContactName`))";
			}
		}
